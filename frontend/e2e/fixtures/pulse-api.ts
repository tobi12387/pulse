  ...
  return {
    todayOptions: {
        ...
        options: [
          ...
          {
            id: 'planned-easier-default',
            kind: 'workout',
            priority: 'secondary',
            title: 'Leichtere Alternative',
            detail: '55 min Z1, falls Warm-up oder Kopf nicht passen.',
            cta: 'Plan anpassen',
            targetPath: '/plan?tab=training&source=today-change&intent=easier&workoutId=planned-default#next-training-decision',
            evidence: ['Readiness 82/100', 'TSB 3.0'],
            activityType: 'bike',
            zone: 1,
            durationMin: 55,
  ...
  await page.route('**/*', async (route) => {
    ...
    if (url.pathname === '/api/pulse/nutrition' && request.method() === 'POST') {
      ...
      return json(route, {
        ...
        ambientTempC: body.ambientTempC ?? null,
        sweatRateLPerHour: body.sweatRateLPerHour ?? null,
