...
export const pulseApi = {
  ...
  fuelingRecovery: {
    ...
    debt: (): Promise<{ fuelingDebt: PulseFuelingDebtSummary; outcomeBaseline: PulseFuelingOutcomeBaseline }> =>
  ...
  goalProjection: (horizonDays = 180): Promise<PulseGoalProjectionResponse> =>
    request(`/goal-projection?horizonDays=${encodeURIComponent(String(horizonDays))}`),
  ...
  insights: {
    get: (domain: string, days = 30, refresh = false): Promise<{
      ...
      evidence?: Array<{ label: string; value: string; window: string; status: 'available' | 'limited' | 'missing'; targetRoute?: '/data' | '/data?tab=analysis' | '/data?tab=analysen' | '/plan' | '/insights' | `/activities/${number}`; targetLabel?: string }>;
