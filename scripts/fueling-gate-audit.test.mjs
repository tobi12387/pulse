...
test('fueling gate audit names existing long carb logs before new logs', () => {
  ...
  assert.match(packet, /Use this only as a worksheet while saving each Activity Fueling target in the UI:/);
  assert.match(packet, /GI UI entry worksheet:/);
  ...
  assert.match(nextPrompt, /If you already remember the real GI responses for existing candidates, use this as a UI entry worksheet while opening each target:/);
  assert.match(nextPrompt, /GI UI entry worksheet:/);
  ...
  assert.match(nextPrompt, /A chat answer or worksheet is not evidence capture until the value is saved in Pulse/);
  ...
  assert.match(captureChecklist, /Use this only as a worksheet while saving each Activity Fueling target in the UI:/);
  assert.match(captureChecklist, /GI UI entry worksheet:/);
