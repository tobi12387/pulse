...
- Runtime note: server mirror was verified on `848fe0c`; the app-runtime comparison target for iPhone/PWA field evidence remains the observed Settings `App-Stand`, not a copied expected value.
...
- Real iPhone/PWA field evidence against server commit `848fe0c`, using the observed Settings `App-Stand` as the runtime evidence value.
