...
2026-05-21 route evidence captured 9 desktop and 15 mobile screenshots and found one small Plan mobile scenario preview-label issue: English `Preview-only` wrapped in the first preview card. The issue was handled as a narrow `Trainingsanpassung` support fix with `Nur Vorschau`; after the fix, the refreshed evidence shows 0 horizontal overflow and no additional daily-flow friction. See `docs/qa/2026-05-21-performance-os-next-evidence.md` and `docs/qa/2026-05-21-plan-preview-label-evidence.md` before proposing new UI/UX work from current route state.
