...
These seeds keep the next package shape ready after manual evidence lands. They are not current product cards and do not override the gate above: while the gate is closed, product coding remains deferred.
...
1. **Nutrition trend summaries** (`Lernschleifen`)
   ...
   Outcome: Data turns repeated long-session Fueling evidence into a conservative MacroFactor-style learning summary, while Home and Plan use it only when the existing learning calibration makes it relevant to today's action or an explicit weekly decision.
   ...
   - Show the trend first in Data as learning evidence with compact evidence/gap rows; keep Home and Plan quiet unless the shared learning calibration classifies the signal as `today_action` or true weekly decision input.
...
2. **iPhone/PWA field reliability** (platform reliability support)
   ...
   - Patch only the observed PWA, certificate, offline, Push or Settings diagnostics gap; do not introduce native/wrapper scope unless the field record proves unresolved recurring PWA/VPN failure.
...
No ungated product package is currently queued after the Home quiet-continuity follow-up. Do not start product coding from stale history; first replenish this backlog from the roadmap, fresh route evidence or Tobi's explicit direction. The roadmap's remaining named implementation themes are gated: Nutrition trend summaries need enough comparable complete logs, and iPhone/PWA field reliability needs real-device evidence. Refresh all current blockers with `npm run audit:performance-gates -- --today <YYYY-MM-DD>`; the combined audit promotes the first open `Next unblock` before the per-gate details, and JSON includes structured `nextUnblock.metadata` for paths/options/Fueling candidate context/evidence checklists/runbooks plus the iPhone/PWA Settings field URL. Use `npm run audit:performance-gates -- --today <YYYY-MM-DD> --packet` when handing off all open manual gates in order, or `npm run audit:performance-checklist -- --today <YYYY-MM-DD>` for a shorter checkbox handoff; from feature branches, handoff modes auto-defer server verification while planning, default the expected commit to `origin/main`, and show that local-planning state in the output. Add `--local-planning` manually only for non-handoff planning summaries that also need this behavior. Rerun the normal audit from clean `main` before deploy-sensitive or current iPhone field claims. Add `--expected-commit <short>` only when intentionally pinning a known deployed/runtime commit for a field run. Use `npm run audit:performance-next -- --today <YYYY-MM-DD>` for a concise first-action handoff and `--fail-on-gated` only for automation that should fail on any open gate. For focused follow-up, use `npm run audit:fueling-gate -- --today <YYYY-MM-DD>` and `npm run audit:iphone-pwa-gate`; the iPhone packet/scaffold/next-prompt should be opened from the printed `/settings?section=device` field-proof URL before recording Device, iOS and `App-Stand`. A 2026-05-21 local nutrition DB audit found 4 during logs and 0/3 comparable complete Fueling logs; two existing long carb logs need structured GI comfort before they can count, and one new complete long-session log is still needed after that.

2026-05-21 route evidence first captured 9 desktop and 15 mobile screenshots and found narrow first-viewport friction, but no new package-sized backlog: the Plan mobile scenario preview label now says `Nur Vorschau`, Home learning calibration no longer steals completed-day review or leaves calibration CTAs with generic fallback detail, Home translates primary learning-calibration actions as `Lernschleife`/`Muster pruefen` instead of analysis jargon, and Activity Fueling deep links now focus the GI comfort action group when that is the missing evidence. Later current-main route evidence at `2f41418` in `docs/qa/2026-05-21-current-route-evidence-2f41418.md` extended the `ae3285d`/`93e6036` route passes: 9 desktop and 17 mobile screenshots still had 0 horizontal overflow, the Activity Fueling closure path uses sport-neutral `langen During-Log` copy for long endurance logs, Fueling readiness exposes both existing completion candidates in app/audit while preserving the 3/3 trend gate, the iPhone field packet and Performance gate metadata carry current-commit iPhone rerun commands, and the refreshed audit keeps Fueling and iPhone/PWA field evidence gated. The iPhone/PWA audit now also treats stale `Server commit under test` field evidence as a gate gap, so the old `9e05189` manual record cannot prove the current expected commit; use the live audit command for the exact current hash instead of creating docs-only hash-refresh PRs. Simulated iPhone WebKit evidence at `9b6c7d7` fixed a concrete Home stage-strip `JETZT` clipping issue and passed the bounded WebKit PWA gate, but it does not replace real-device iPhone/VPN/PWA field evidence. No additional daily-flow or Fueling UI/UX slice is currently justified. See `docs/qa/2026-05-21-performance-os-next-evidence.md`, `docs/qa/2026-05-21-plan-preview-label-evidence.md`, `docs/qa/2026-05-21-home-calibration-review-clarity.md`, `docs/qa/2026-05-21-post-fueling-route-evidence.md`, `docs/qa/2026-05-21-current-main-route-evidence.md`, `docs/qa/2026-05-21-current-main-route-evidence-20caf40.md`, `docs/qa/2026-05-21-current-main-evidence-0d8cd63.md`, `docs/qa/2026-05-21-current-main-evidence-ae3285d.md`, `docs/qa/2026-05-21-current-main-evidence-ec77c26.md`, `docs/qa/2026-05-21-current-main-route-evidence-9ac97fc.md`, `docs/qa/2026-05-21-current-route-evidence-2f41418.md`, `docs/qa/2026-05-21-current-route-evidence-f136cc0.md`, `docs/qa/2026-05-21-current-route-evidence-53f0dd1.md`, `docs/qa/2026-05-21-current-gates-4c046ab.md`, `docs/qa/2026-05-21-current-gates-5477d49.md`, `docs/qa/2026-05-21-iphone-field-commit-freshness.md`, `docs/qa/2026-05-21-iphone-webkit-stage-strip.md`, `docs/qa/2026-05-21-activity-fueling-action-focus.md`, `docs/qa/2026-05-21-home-learning-language.md`, `docs/qa/2026-05-21-fueling-long-session-copy.md`, `docs/qa/2026-05-21-fueling-evidence-checklist.md`, `docs/qa/2026-05-21-iphone-field-checklist-audit.md`, `docs/qa/2026-05-21-iphone-field-recovery-packet.md`, `docs/qa/2026-05-21-performance-iphone-recovery-metadata.md` and `docs/qa/2026-05-21-server-runbook-gate-render.md` before proposing new UI/UX work from current route state.

Fresh current-main route evidence at `53f0dd1` in `docs/qa/2026-05-21-current-route-evidence-53f0dd1.md` again captured 9 desktop and 17 mobile screenshots with 0 horizontal overflow. Manual review of Home, Data Fueling, Activity Fueling, mental quick check and Plan scenario preview found no new UI/UX implementation slice; the current blockers remain manual Fueling GI comfort evidence and real iPhone/PWA evidence.

2026-06-03 top-app follow-up evidence in `docs/qa/2026-06-03-mobile-chrome-solid.md` and `docs/qa/2026-06-03-desktop-focus-surface.md` captured 9 desktop and 17 mobile screenshots with 0 horizontal overflow after the mobile chrome was made opaque and the redundant desktop Workspace-Topbar was removed. Manual review found the latest shell calmer and clearer, but did not open a new autonomous product package; the current blockers remain Fueling GI comfort evidence and real iPhone/PWA evidence.
