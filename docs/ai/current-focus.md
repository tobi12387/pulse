...
- Training-plan intelligence is now controlled by persisted capability levels, deterministic archetypes, TrainNow options, scenario preview/apply, season load modeling, Garmin sync contracts and goal limiter evidence. Garmin Execution Ledger and Adaptation Event Queue are implemented as the first benchmark-upgrade foundations: Garmin execution attempts are recorded non-blockingly, and write-triggered adaptation events centralize activity/RPE/mental/fueling/recovery/sync-debt reasons for Home and Plan. The benchmark-upgrade roadmap remains active in `docs/superpowers/plans/2026-05-10-training-benchmark-upgrade-roadmap.md`; implement next in order: Workout Library v2, Mobile Plan Flow, Power Data Quality Foundation, Power Duration/Durability, Season ATP v2, Strength/Mobility Companion.
