...
- Training-plan intelligence is now controlled by persisted capability levels, deterministic archetypes, TrainNow options, scenario preview/apply, season load modeling, Garmin sync contracts and goal limiter evidence. Plan surfaces weekly decision reasons as signal groups. The next benchmark-upgrade roadmap is active in `docs/superpowers/plans/2026-05-10-training-benchmark-upgrade-roadmap.md`; implement in order: Garmin Execution Ledger, Adaptation Event Queue, Workout Library v2, Mobile Plan Flow, Power Data Quality Foundation, Power Duration/Durability, Season ATP v2, Strength/Mobility Companion.
