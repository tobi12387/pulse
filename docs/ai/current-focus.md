...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch. No ungated product package is queued after the current Home quiet-continuity follow-up; next autonomous work should first replenish/reprioritize that backlog from the roadmap, fresh route evidence or Tobi's explicit direction.
- Shipped baselines:
