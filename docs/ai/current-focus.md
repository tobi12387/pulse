...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch. No ungated product package is queued after the current Home quiet-continuity follow-up. Fresh 2026-05-21 route evidence found one small Plan mobile preview-label friction, now covered as `Nur Vorschau`; after that fix it shows 0 horizontal overflow and no additional UI/UX workflow slice. Simulated iPhone WebKit evidence at `0feb515` also found no horizontal overflow or concrete first-viewport blocker. Next autonomous product work still needs roadmap evidence, new route/user friction, nutrition logs, iPhone/PWA real-device evidence or Tobi's explicit direction.
- Shipped baselines:
