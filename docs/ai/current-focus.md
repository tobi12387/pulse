...
- For faster delivery, start with `npm run delivery:intake`, iterate with `npm run verify:<track>:fast`, then run `npm run delivery:manifest` and copy its PR-body fields instead of rediscovering scope, lane, checks, auto-merge and deploy manually.
