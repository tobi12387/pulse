...
- Latest verified server runtime recorded in this file before the current branch: `0d6bd88` / PR #298, deployed 2026-05-10. Verify live deploy state with `scripts/verify-server.sh` when it matters.
...
- Current navigation baseline is Heute (`/`), Data, Plan and Settings, with Insights inside Data and `/coach` as a compatibility/deep-link route. Future tabs, areas or modes are allowed when they make a recurring flow clearer or calmer; validate with route evidence before changing primary navigation.
...
- Training-plan intelligence is now controlled by persisted capability levels, deterministic archetypes, TrainNow options, scenario preview/apply, season load modeling, Garmin sync contracts and goal limiter evidence. Garmin Execution Ledger, Adaptation Event Queue, Workout Library v2, Mobile Plan Flow, Power Data Quality Foundation, Power Duration/Durability, Season ATP v2 and Strength/Mobility Companion are implemented as benchmark-upgrade foundations. Fresh Benchmark UI Roadmap Phase 1 (UI Accessibility Polish v2), Phase 2 (Daily Command Center v2), Garmin Execution Trust v2, Progression Library v2, Plan Refresh Preview v1, Plan Apply + Garmin Readback Closure, Today Options Signal Labels, Fueling Debt Closure, Limiter-To-Workout Mapping and the no-Garmin-write Plan QA harness are implemented. Completed 2026-05-10 benchmark and implementation plans have been archived under `docs/superpowers/plans/completed/`. The first UX Task Contract slice is implemented on the daily decision card, the root navigation label is now `Heute` while `/` remains stable, Plan starts with a primary action contract before evidence/tools, Home has a read-only Daily Delta v1 card for plan-vs-execution closure, Garmin execution closure polish now verifies readback repeat counts before showing `ready`, Athlete-Level Decision Language v1 translates workout fit into `Machbar`/`Produktiv`/`Stretch`/`Zu hart heute` daily actions, Scenario Preview Capability Fit shows projected archetype/workout-level/fit before apply without Garmin writes, Nutrition Intelligence v1 turns recent low-intake GI logs into a concrete controlled `50-70 g/h` long-session target with 750-ml bottle/powder math, and Nutrition Outcome Baseline v1 structures recent long-session logs for carb target, bottles, powder, fluid and sodium evidence gaps inside Activity and Plan. Next autonomous work should move to optional Daily Delta Plan/Data echoes or Garmin modal wording polish only if route evidence shows confusion.
...
- Historical benchmark/UI roadmap evidence lives in `docs/qa/2026-05-10-fresh-benchmark-ui-review.md` and `docs/qa/2026-05-10-post-progression-benchmark-ui-review.md`; active product ordering now lives in the canonical roadmap.
