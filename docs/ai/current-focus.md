...
- Fresh 2026-05-12 benchmark ordering is captured in the canonical roadmap. Plan Change Inbox v1 and Today Change Flow v1 are merged/deployed; Workout Progression Clarity v3 is implemented in the current PR slice so `/plan` explains progression role, calibration, repetition rationale and change triggers without plan/Garmin writes.
- Next non-gated roadmap candidates are Garmin Execution Chain UI, Weekly Coach Review, then Recovery & Mental Resilience. Nutrition Trend Summaries remain data-gated, and Mobile Field Reliability remains a real-iPhone gate.
