...
- First-pass and second-pass Performance-OS baselines across `Tagesentscheidung`, `Trainingsanpassung` and `Lernschleifen` are shipped; Track 1 and Track 2 also have their third-pass learning-calibration packages. Track 1 now has fourth- through eighth-pass packages for the Home body/goal/everyday tradeoff, completed-day learning closure, today-only tradeoff learning, resolved tradeoff quietness and fresh Home reopen explanations, Track 2 carries repeated Tageskonflikte into the weekly decision receipt, consumes classified tradeoff patterns only for real weekly decisions, keeps handled tradeoff receipts quiet until fresh weekly evidence appears and now names fresh reopen sources in Plan, and Track 3 classifies tradeoff evidence as Home action, Plan decision, resolved quiet context or evidence gap, names fresh reopen evidence while old handled history stays context and groups repeated reopen sources into trend-level learning.
