...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch. No ungated product package is queued after the current Home quiet-continuity follow-up. Fresh 2026-05-21 route evidence found and closed two narrow UI/contract issues: the Plan mobile preview label now says `Nur Vorschau`, and Home no longer lets learning calibration confuse completed-day review or CTA detail copy. Refreshed evidence at `2dbafcc` plus this branch shows 9 desktop and 15 mobile screenshots with 0 horizontal overflow and no additional UI/UX workflow slice. A 2026-05-21 local nutrition DB audit still shows 0/3 comparable complete Fueling logs; two existing long carb logs can become evidence when GI comfort is added, then one new complete long-session log is still needed. Simulated iPhone WebKit evidence at `0feb515` also found no horizontal overflow or concrete first-viewport blocker. Next autonomous product work still needs roadmap evidence, new route/user friction, completed nutrition logs, iPhone/PWA real-device evidence or Tobi's explicit direction.
- Shipped baselines:
