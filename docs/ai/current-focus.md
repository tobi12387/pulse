...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Lernschleifen:** explain tradeoff patterns as action, plan decision or watch context.
  2. **Tagesentscheidung:** use repeated tradeoff learning only when it changes the daily answer.
  3. **Trainingsanpassung:** use classified tradeoff patterns only when they change the weekly decision.
- Shipped baselines:
