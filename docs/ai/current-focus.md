...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Tagesentscheidung:** keep resolved tradeoff learning out of the daily lead unless fresh evidence changes today.
  2. **Trainingsanpassung:** keep handled tradeoff receipts quiet until fresh weekly evidence appears.
  3. **Lernschleifen:** explain why resolved tradeoffs reopen only when fresh evidence appears.
- Shipped baselines:
  ...
  - **Lernschleifen** delivered Data action contracts, Fueling learning-loop copy, Home watch-context gating, the Data training-risk contract, the Data learning-calibration gate for Decision Quality, Personal Response and Fueling trends, tradeoff pattern classification as action, plan decision or watch context, and resolved tradeoff quietness until fresh evidence appears.
