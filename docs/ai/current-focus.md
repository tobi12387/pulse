...
- Fueling & Recovery preference capture, the pure backend guidance engine, read-only Plan modal guidance, Garmin workout-description handoff, and generic portion equivalents are implemented: no dietary restrictions, Ministry products are the preferred anchor, Pulse may propose conservative carb/hour, sodium and body-weight-based guidance. Next work should add optional product calibration only after concrete Ministry serving data exists, or use completed-session fueling logs to learn tolerance and plan adjustments.
