...
- Performance Operating System spec is merged and Tobi has granted autonomous follow-up execution. Daily Intelligent Action Contract v2 enriches Home's decision details with top signals, goal impact, Garmin execution state and safest option; Daily Decision Continuity v1 adds why the recommendation still holds or what changed from today's Plan-vs-Garmin delta. Daily Garmin Execution Signal v1 lets local/template-only/degraded/blocked execution state become a `Garmin` signal for open planned workouts and route the CTA to Plan > Ausfuehrung without automatic Garmin writes. Daily Fueling Signal v1 lets open Today-Options Fueling/GI debt influence Home's decision contract without unlocking trend summaries. Daily Fueling Learning Signal v1 lets existing Fueling Outcome Baseline readiness become a `Fueling-Lernen` signal for open long/fueling-relevant workouts and route the CTA to the existing Plan workout fueling guidance while trend summaries stay gated. Daily Post-Workout Fueling Closure v1 lets completed long/fueling-relevant planned or off-plan activities keep an open Home step to the existing Activity Fueling Log while Fueling learning readiness still lacks comparable complete During logs. Daily Goal Pressure Signal v1 lets the existing Goal Projection add top-goal probability/evidence status and next intervention to Home's decision details. Daily Goal Impact Specificity v1 lets that Goal Projection also sharpen the `Zielwirkung` row with top goal, probability/evidence status and next intervention. Daily Adaptation Signal v1 lets the primary open Plan Adaptation event become a prioritized `Anpassung` signal in Home's decision contract. Daily Analysis Signal v1 lets existing Training Analytics Durability become a prioritized `Analyse` signal for open planned workouts and route the Daily Decision CTA to Data > Analyse when it is the leading factor. Daily Leading Signal Action v1 lets operative Contract signals steer the open Daily Decision CTA/result path when no explicit Next-Best-Action owns the click; completed feedback flows keep their structured steps. Daily Mental Boundary v1 lets a saved non-stable Mental Check-in add the day's protective/sensitive boundary to Home's decision contract. Daily Data Confidence v1 lets non-ready Garmin/profile data add a `Daten` trust signal to Home's decision contract. Daily Decision Quality Signal v1 lets stale/watch/strategy-change `/decisions/quality` evidence become a prioritized `Lernen` signal in Home's decision contract. Daily Recovery Pressure Signal v1 lets severe sleep debt, HRV decline, elevated RHR or low recovery score become a prioritized `Recovery` signal in Home's decision contract. Daily Signal Saturation/Priority/Leading-Factor Guard v1 prevents the optional decision details from hard-capping active data, fueling, mental, goal and training signals, orders them by decision relevance before baseline values, names the first factor as `Heute entscheidet` and surfaces that factor, `Seit letzter Entscheidung` and `Sicherste Option` on the main Daily Decision before expansion. Everyday Adaptation Inbox v1 gives Plan one no-write entry point for less time, not-ready, done-differently and skip decisions. Analysis Translation v1 makes Data > Analyse name the currently actionable deep signal and the interesting-but-not-actionable evidence gap. Performance OS Route Evidence Pass on `775c46a` found no immediate first-viewport restructuring need; Nutrition Learning Readiness v1 keeps trend summaries gated but makes the evidence count and missing structured fields visible.
- Daily Personal Response Signal v1 lets existing Personal Response evidence become a cautious `Reaktion` signal for open planned workouts and route the Home Daily Decision CTA to Data > Analyse / Reaktionsmuster. Current-day Mental Boundary, Recovery, Goal, Garmin and Fueling signals keep priority over historical response patterns.
...
- No next broad runtime UI/UX slice is selected from current route evidence. The Home decision contract now carries continuity from today's `daily-delta`, open Garmin execution gaps for planned workouts, Personal Response patterns for open planned workouts, open Fueling/GI debt from Today Options, Fueling Outcome Baseline learning readiness for open long/fueling-relevant workouts and completed long/fueling-relevant activities, decision-quality learning from `/decisions/quality`, recovery pressure from existing `PulseRecoveryMetrics`, primary open Plan Adaptation events, power-duration Durability analysis from Training Analytics for open planned workouts, top-goal pressure and goal-impact specificity from Goal Projection, non-stable Mental Check-in boundaries and stale/partial/empty data-confidence warnings without hard-capping the optional signal list; the main Daily Decision surfaces `Heute entscheidet`, `Seit letzter Entscheidung` and `Sicherste Option` before expansion, and the primary CTA follows operative leading signals for open decisions when no explicit Next-Best-Action owns the click. Completed long/fueling-relevant activities can keep `Fueling-Log pruefen` open until comparable complete During evidence is captured. Optional details keep the full prioritized signal list. Nutrition Trend Summaries remain data-gated with visible `Trend-Evidenz x/3` readiness in Fueling baselines, and Mobile Field Reliability remains a real-iPhone gate.
