...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Lernschleifen:** explain why resolved tradeoffs reopen only when fresh evidence appears.
  2. **Tagesentscheidung:** explain fresh tradeoff reopens without replaying old resolved history.
  3. **Trainingsanpassung:** explain fresh Plan tradeoff reopens by evidence source.
- Shipped baselines:
