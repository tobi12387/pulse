...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Lernschleifen:** close reopen-source trends after weekly decisions.
- Shipped baselines:
  ...
  - **Lernschleifen** delivered Data action contracts, Fueling learning-loop copy, Home watch-context gating, the Data training-risk contract, the Data learning-calibration gate for Decision Quality, Personal Response and Fueling trends, tradeoff pattern classification as action, plan decision or watch context, resolved tradeoff quietness until fresh evidence appears, source-specific fresh reopen explanations and repeated reopen-source trend grouping.
