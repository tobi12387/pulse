...
- First-pass and second-pass Performance-OS baselines across `Tagesentscheidung`, `Trainingsanpassung` and `Lernschleifen` are shipped; Track 1 and Track 2 also have their third-pass learning-calibration packages. Track 1 now has fourth-, fifth- and sixth-pass packages for the Home body/goal/everyday tradeoff, completed-day learning closure and today-only tradeoff learning, Track 2 carries repeated Tageskonflikte into the weekly decision receipt and consumes classified tradeoff patterns only for real weekly decisions, and Track 3 classifies tradeoff evidence as Home action, Plan decision or watch context.
...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Lernschleifen:** keep resolved tradeoff patterns quiet unless fresh evidence changes the action.
  2. **Tagesentscheidung:** keep resolved tradeoff learning out of the daily lead unless new evidence changes today.
  3. **Trainingsanpassung:** keep handled tradeoff receipts quiet until fresh weekly evidence appears.
- Shipped baselines:
  - **Tagesentscheidung** delivered Home completed-day learning, Activity closure evidence, the local Daily Decision signal registry, the last-decision follow-up signal and gated Data learning calibration in Home; the fourth-pass package makes Home lead with one `Tageskonflikt` when body, goal and everyday option disagree, the fifth pass makes completed Tageskonflikt days learnable through existing Activity feedback, and the sixth pass uses repeated tradeoff learning only when it changes today's adaptive option.
  - **Trainingsanpassung** delivered the shared Plan weekly decision contract, preview-only Accept/Adapt/Defer controls, Home/Data handoffs, local weekly decision receipts, gated learning calibration in explicit weekly decisions, repeated Tageskonflikte in the weekly decision receipt and classifier-gated weekly tradeoff consumption.
  - **Lernschleifen** delivered Data action contracts, Fueling learning-loop copy, Home watch-context gating, the Data training-risk contract, the Data learning-calibration gate for Decision Quality, Personal Response and Fueling trends, and tradeoff pattern classification as action, plan decision or watch context.
