...
- Adaptive Season Builder v1 is implemented: Plan > Training now shows a read-only `Saisonvertrag` that combines Season Strategy and Goal Projection evidence without hidden plan/Garmin writes.
...
- Fresh 2026-05-12 benchmark ordering is now captured in the canonical roadmap. Plan Change Inbox v1 and Today Change Flow v1 are implemented locally in the current PR slice: `/plan` centralizes refresh preview, adaptation events and Garmin sync debt; planned-day Home alternatives route to the existing workout decision via `source=today-change` instead of custom workout creation.
- Next non-gated roadmap candidates are Workout Progression Clarity v3, Garmin Execution Chain UI, Weekly Coach Review, then Recovery & Mental Resilience. Nutrition Trend Summaries remain data-gated, and Mobile Field Reliability remains a real-iPhone gate.
