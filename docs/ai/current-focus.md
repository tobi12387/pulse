...
- Training-plan intelligence is now controlled by persisted capability levels, deterministic archetypes, TrainNow options, scenario preview/apply, season load modeling, Garmin sync contracts and goal limiter evidence. Garmin Execution Ledger is implemented as the first benchmark-upgrade foundation: upload/delete attempts are recorded non-blockingly and Plan can explain the latest Garmin execution entry. The benchmark-upgrade roadmap remains active in `docs/superpowers/plans/2026-05-10-training-benchmark-upgrade-roadmap.md`; implement next in order: Adaptation Event Queue, Workout Library v2, Mobile Plan Flow, Power Data Quality Foundation, Power Duration/Durability, Season ATP v2, Strength/Mobility Companion.
