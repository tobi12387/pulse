...
- First-pass and second-pass Performance-OS baselines across `Tagesentscheidung`, `Trainingsanpassung` and `Lernschleifen` are shipped; Track 1 and Track 2 also have their third-pass learning-calibration packages. Track 1 now has fourth- through tenth-pass packages for the Home body/goal/everyday tradeoff, completed-day learning closure, today-only tradeoff learning, resolved tradeoff quietness, fresh Home reopen explanations, reopen-source trend context and closed source-trend continuity, Track 2 carries repeated Tageskonflikte into the weekly decision receipt, consumes classified tradeoff patterns only for real weekly decisions, keeps handled tradeoff receipts quiet until fresh weekly evidence appears, names fresh reopen sources in Plan, uses reopen-source trends as weekly decision context and keeps closed source trends as quiet weekly receipts, and Track 3 classifies tradeoff evidence as Home action, Plan decision, resolved quiet context or evidence gap, names fresh reopen evidence while old handled history stays context, groups repeated reopen sources into trend-level learning, closes handled source trends after weekly decisions and shows weekly receipt source trends as learning confidence.
