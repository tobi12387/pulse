...
- The shipped Performance-OS baseline already covers the three tracks: `Tagesentscheidung`, `Trainingsanpassung` and `Lernschleifen`. Detailed pass history and gate-open package seeds live in `docs/ai/next-product-packages.md`; do not reconstruct them from this snapshot.
