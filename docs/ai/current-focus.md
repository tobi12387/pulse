...
- Garmin Execution Chain UI is implemented in the current PR slice: `/plan?tab=execution` shows template, calendar, readback, repeats and execution as one compact chain plus one next action, without automatic Garmin writes on load.
- Next non-gated roadmap candidates are Weekly Coach Review, then Recovery & Mental Resilience. Nutrition Trend Summaries remain data-gated, and Mobile Field Reliability remains a real-iPhone gate.
