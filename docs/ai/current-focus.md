...
- No next broad runtime UI/UX slice is selected from current route evidence. The Home decision contract now carries continuity from today's `daily-delta` and open Fueling/GI debt from Today Options; Nutrition Trend Summaries remain data-gated with visible `Trend-Evidenz x/3` readiness in Fueling baselines, and Mobile Field Reliability remains a real-iPhone gate.
