...
- First-pass and second-pass Performance-OS baselines across `Tagesentscheidung`, `Trainingsanpassung` and `Lernschleifen` are shipped; Track 1 and Track 2 also have their third-pass learning-calibration packages. Track 1 now has fourth-, fifth-, sixth- and seventh-pass packages for the Home body/goal/everyday tradeoff, completed-day learning closure, today-only tradeoff learning and resolved tradeoff quietness, Track 2 carries repeated Tageskonflikte into the weekly decision receipt and consumes classified tradeoff patterns only for real weekly decisions, and Track 3 classifies tradeoff evidence as Home action, Plan decision, resolved quiet context or evidence gap.
...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Trainingsanpassung:** keep handled tradeoff receipts quiet until fresh weekly evidence appears.
  2. **Lernschleifen:** explain why resolved tradeoffs reopen only when fresh evidence appears.
  3. **Tagesentscheidung:** explain fresh tradeoff reopens without replaying old resolved history.
- Shipped baselines:
