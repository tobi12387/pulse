...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Tagesentscheidung:** use repeated tradeoff learning only when it changes the daily answer.
  2. **Trainingsanpassung:** use classified tradeoff patterns only when they change the weekly decision.
  3. **Lernschleifen:** keep resolved tradeoff patterns quiet unless fresh evidence changes the action.
- Shipped baselines:
