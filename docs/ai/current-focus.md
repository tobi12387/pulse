...
- Current hard Performance-OS backlog order is kept as ready package cards in `docs/ai/next-product-packages.md` so agents do not re-plan from scratch:
  1. **Trainingsanpassung:** use classified tradeoff patterns only when they change the weekly decision.
  2. **Lernschleifen:** keep resolved tradeoff patterns quiet unless fresh evidence changes the action.
  3. **Tagesentscheidung:** keep resolved tradeoff learning out of the daily lead unless new evidence changes today.
- Shipped baselines:
  ...
  - **Trainingsanpassung** delivered the shared Plan weekly decision contract, preview-only Accept/Adapt/Defer controls, Home/Data handoffs, local weekly decision receipts, gated learning calibration in explicit weekly decisions and repeated Tageskonflikte in the weekly decision receipt.
