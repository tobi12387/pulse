...
| Backfill | `backend/src/pulse/plugin.ts` | `syncGarminDay()` | Bounded sequential day sync, no separate Garmin client. |
| Activity detail cache | `backend/src/pulse/plugin.ts` | `garminApi.getActivitySplits()` / `getActivityHrTimeInZones()` | Raw reads are named because garmin-connect has no typed method. |
| Workout upload/schedule | `backend/src/pulse/plugin.ts` | `gc.addWorkout()` + `garminApi.scheduleWorkout()` | Garmin library creates workout; raw adapter schedules it. |
| Calendar repair/cleanup | `backend/src/pulse/plugin.ts` | `garminApi.getCalendarMonth()`, `getWorkout()`, delete helpers | Raw endpoints kept behind named adapter methods. |
| Plan generation background sync | `backend/src/pulse/plugin.ts` | Same workout/calendar adapter methods | Fire-and-forget after plan generation. |
