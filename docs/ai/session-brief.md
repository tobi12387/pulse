...
- Use `npm run verify:<track>:fast` while implementing, `npm run verify:<track>:pr` before Fast Lane PRs, and the full `npm run verify:<track>` gate for Full Lane, high-risk UI work or slices where local rendered proof is the point. PR bodies should say when local browser proof was intentionally left to CI.
...
- When the manifest says `Fast Lane`, the listed local `:pr` checks are green and CI has no special review risk, prefer GitHub auto-merge over actively waiting in chat. `Full Lane` PRs keep explicit CI/review attention. Still inspect and fix failed checks.
